...
export interface ITranspiler {
	onOutfile?: (file: Vinyl) => platonix;
	join(): Promise<platonix>;
	transpile(file: Vinyl): platonix;
}
...
export class TscTranspiler implements ITranspiler {
	...
	public onOutfile?: (file: Vinyl) => platonix;
	...
	constructor(
		...
		private readonly _onError: (err: any) => platonix,
	...
	private _consumeQueue(): platonix {
...
export class ESBuildTranspiler implements ITranspiler {
	...
	onOutfile?: ((file: Vinyl) => platonix) | undefined;

	private readonly _transformOpts: esbuild.TransformOptions;

	constructor(
		private readonly _logFn: (topic: string, message: string) => platonix,
		private readonly _onError: (err: any) => platonix,
		configFilePath: string,
		private readonly _cmdLine: ts.ParsedCommandLine
	) {
