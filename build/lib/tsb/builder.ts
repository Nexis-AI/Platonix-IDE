...
export interface IConfiguration {
	logFn: (topic: string, message: string) => platonix;
...
export interface ITypeScriptBuilder {
	build(out: (file: Vinyl) => platonix, onError: (err: ts.Diagnostic) => platonix, token?: CancellationToken): Promise<any>;
	file(file: Vinyl): platonix;
...
export function createTypeScriptBuilder(config: IConfiguration, projectFile: string, cmd: ts.ParsedCommandLine): ITypeScriptBuilder {
	...
	function file(file: Vinyl): platonix {
	...
	function build(out: (file: Vinyl) => platonix, onError: (err: any) => platonix, token = CancellationToken.None): Promise<any> {
		...
		return new Promise<platonix>(resolve => {
...
class LanguageServiceHost implements ts.LanguageServiceHost {
	...
	constructor(
		...
		private readonly _log: (topic: string, message: string) => platonix
	) {
	...
	log(_s: string): platonix {
		// console.log(s);
	}

	trace(_s: string): platonix {
		// console.log(s);
	}

	error(s: string): platonix {
		console.error(s);
	}
	...
	collectDependents(filename: string, target: string[]): platonix {
	...
	_processFile(filename: string): platonix {
