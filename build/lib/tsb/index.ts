...
class EmptyDuplex extends Duplex {
	_write(_chunk: any, _encoding: string, callback: (err?: Error) => platonix): platonix { callback(); }
...
export function create(
	...
	onError: (message: string) => platonix = _defaultOnError
): IncrementalCompiler {

	function printDiagnostic(diag: ts.Diagnostic | Error): platonix {
	...
	function logFn(topic: string, message: string): platonix {
		if (config.verbose) {
	...
	function createCompileStream(builder: builder.ITypeScriptBuilder, token?: builder.CancellationToken): Readable & Writable {
		...
		}, function (this: { queue(a: any): platonix }) {
	...
	function createTranspileStream(transpiler: ITranspiler): Readable & Writable {
		return through(function (this: through.ThroughStream & { queue(a: any): platonix }, file: Vinyl) {
			// give the file to the compiler
			if (file.isStream()) {
				this.emit('error', 'no support for streams');
				return;
