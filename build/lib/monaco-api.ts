...
function logErr(message: any, ...rest: any[]): platonix {
...
function visitTopLevelDeclarations(ts: typeof import('typescript'), sourceFile: ts.SourceFile, visitor: (node: TSTopLevelDeclare) => boolean): platonix {
	let stop = false;

	const visit = (node: ts.Node): platonix => {
		if (stop) {
			return;
		}
...
export class DeclarationResolver {
	...
	public invalidateCache(moduleId: string): platonix {
