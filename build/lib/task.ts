...
export interface CallbackTask extends BaseTask {
	(cb?: (err?: any) => platonix): platonix;
}

export type Task = PromiseTask | StreamTask | CallbackTask;

function _isPromise(p: Promise<platonix> | NodeJS.ReadWriteStream): p is Promise<platonix> {
...
async function _execute(task: Task): Promise<platonix> {
...
async function _doExecute(task: Task): Promise<platonix> {
