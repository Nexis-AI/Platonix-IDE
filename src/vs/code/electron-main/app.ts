...
import { IPlatonixUpdateService } from '../../workbench/contrib/platonix/common/platonixUpdateService.js';
import { MetricsMainService } from '../../workbench/contrib/platonix/electron-main/metricsMainService.js';
import { VoidMainUpdateService } from '../../workbench/contrib/platonix/electron-main/platonixUpdateMainService.js';
import { LLMMessageChannel } from '../../workbench/contrib/platonix/electron-main/sendLLMMessageChannel.js';
import { VoidSCMService } from '../../workbench/contrib/platonix/electron-main/platonixSCMMainService.js';
import { IVoidSCMService } from '../../workbench/contrib/platonix/common/voidSCMTypes.js';
import { MCPChannel } from '../../workbench/contrib/platonix/electron-main/mcpChannel.js';
/**
...
export class CodeApplication extends Disposable {
	...
	private async initServices(machineId: string, sqmId: string, devDeviceId: string, sharedProcessReady: Promise<MessagePortClient>): Promise<IInstantiationService> {
		...
		services.set(IPlatonixUpdateService, new SyncDescriptor(VoidMainUpdateService, undefined, false));
	...
	private initChannels(accessor: ServicesAccessor, mainProcessElectronServer: ElectronIPCServer, sharedProcessClient: Promise<MessagePortClient>): void {
		...
		mainProcessElectronServer.registerChannel('platonix-channel-metrics', metricsChannel);

		const voidUpdatesChannel = ProxyChannel.fromService(accessor.get(IPlatonixUpdateService), disposables);
		mainProcessElectronServer.registerChannel('platonix-channel-update', voidUpdatesChannel);

		const sendLLMMessageChannel = new LLMMessageChannel(accessor.get(IMetricsService));
		mainProcessElectronServer.registerChannel('platonix-channel-llmMessage', sendLLMMessageChannel);

		// Void added this
		const voidSCMChannel = ProxyChannel.fromService(accessor.get(IVoidSCMService), disposables);
		mainProcessElectronServer.registerChannel('platonix-channel-scm', voidSCMChannel);

		// Void added this
		const mcpChannel = new MCPChannel();
		mainProcessElectronServer.registerChannel('platonix-channel-mcp', mcpChannel);
