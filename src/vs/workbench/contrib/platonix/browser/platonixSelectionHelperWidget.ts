...
import { mountPlatonixSelectionHelper } from './react/out/platonix-editor-widgets-tsx/index.js';
...
export type PlatonixSelectionHelperProps = {
...
export class SelectionHelperContribution extends Disposable implements IEditorContribution, IOverlayWidget {
	...
	constructor(
		...
		@IPlatonixSettingsService private readonly _platonixSettingsService: IPlatonixSettingsService
	) {
		super();
		...
		this._instantiationService.invokeFunction(accessor => {
			...
			const res = mountPlatonixSelectionHelper(content, accessor);
			if (!res) return;
	...
	private _showHelperForSelection(selection: Selection): void {
		...
		// rerender
		const enabled = this._platonixSettingsService.state.globalSettings.showInlineSuggestions
		...
		if (enabled) {
			this._rerender({ rerenderKey: this._rerenderKey } satisfies PlatonixSelectionHelperProps)
