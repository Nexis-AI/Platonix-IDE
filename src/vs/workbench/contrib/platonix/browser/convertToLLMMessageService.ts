...
class ConvertToLLMMessageService extends Disposable implements IConvertToLLMMessageService {
	...
	constructor(
		...
		@IPlatonixModelService private readonly platonixModelService: IPlatonixModelService,
	...
	private _getVoidRulesFileContents(): string {
		try {
			...
			for (const folder of workspaceFolders) {
				...
				const { model } = this.platonixModelService.getModel(uri)
				if (!model) continue
	...
	private _getCombinedAIInstructions(): string {
		const globalAIInstructions = this.platonixSettingsService.state.globalSettings.aiInstructions;
	...
	prepareLLMSimpleMessages: IConvertToLLMMessageService['prepareLLMSimpleMessages'] = ({ simpleMessages, systemMessage, modelSelection, featureName }) => {
		...
		const { overridesOfModel } = this.platonixSettingsService.state
		...
		const modelSelectionOptions = this.platonixSettingsService.state.optionsOfModelSelection[featureName][modelSelection.providerName]?.[modelSelection.modelName]
	...
	prepareLLMChatMessages: IConvertToLLMMessageService['prepareLLMChatMessages'] = async ({ chatMessages, chatMode, modelSelection }) => {
		...
		const { disableSystemMessage } = this.platonixSettingsService.state.globalSettings;
		const fullSystemMessage = await this._generateChatMessagesSystemMessage(chatMode, specialToolFormat)
		const systemMessage = disableSystemMessage ? '' : fullSystemMessage;

		const modelSelectionOptions = this.platonixSettingsService.state.optionsOfModelSelection['Chat'][modelSelection.providerName]?.[modelSelection.modelName]
