...
import { mountPlatonixCommandBar } from './react/out/platonix-editor-widgets-tsx/index.js'
...
export interface IPlatonixCommandBarService {
...
export const IPlatonixCommandBarService = createDecorator<IPlatonixCommandBarService>('PlatonixCommandBarService');
...
export class PlatonixCommandBarService extends Disposable implements IPlatonixCommandBarService {
	_serviceBrand: undefined;

	static readonly ID: 'void.PlatonixCommandBarService'
	...
	async goToURIIdx(idx: number | null): Promise<void> {
		...
		const { model } = await this._platonixModelService.getModelSafe(nextURI);
		if (!model) return;
...
registerSingleton(IPlatonixCommandBarService, PlatonixCommandBarService, InstantiationType.Delayed); // delayed is needed here :(


export type PlatonixCommandBarProps = {
	uri: URI | null;
	editor: ICodeEditor;
}
...
class AcceptRejectAllFloatingWidget extends Widget implements IOverlayWidget {
	...
	constructor({ editor }: { editor: ICodeEditor, },
		...
		this.instantiationService.invokeFunction(accessor => {
			...
			const res = mountPlatonixCommandBar(root, accessor, { uri, editor } satisfies PlatonixCommandBarProps)
			if (!res) return
			this._register(toDisposable(() => res.dispose?.()))
			this._register(editor.onWillChangeModel((model) => {
				const uri = model.newModelUrl
				res.rerender({ uri, editor } satisfies PlatonixCommandBarProps)
			}))
		})
	}
