...
import { IPlatonixUpdateService } from '../common/platonixUpdateService.js';
import { IWorkbenchContribution, registerWorkbenchContribution2, WorkbenchPhase } from '../../../common/contributions.js';
import * as dom from '../../../../base/browser/dom.js';
import { IUpdateService } from '../../../../platform/update/common/update.js';
import { VoidCheckUpdateRespose } from '../common/platonixUpdateServiceTypes.js';
...
const notifyUpdate = (res: VoidCheckUpdateRespose & { message: string }, notifService: INotificationService, updateService: IUpdateService): INotificationHandle => {
	const message = res?.message || 'This is a very old version of Void, please download the latest version! [Void Editor](https://platonix-ide.com/download-beta)!'
	...
	if (res?.action) {
		...
		if (res.action === 'reinstall') {
			primary.push({
				label: `Reinstall`,
				id: 'platonix.updater.reinstall',
				enabled: true,
				tooltip: '',
				class: undefined,
				run: () => {
					const { window } = dom.getActiveWindow()
					window.open('https://platonix-ide.com/download-beta')
				}
			})
		}
		...
		primary.push({
			...
			run: () => {
				const { window } = dom.getActiveWindow()
				window.open('https://platonix-ide.com/')
			}
		})
...
const notifyErrChecking = (notifService: INotificationService): INotificationHandle => {
	const message = `Void Error: There was an error checking for updates. If this persists, please get in touch or reinstall Void [here](https://platonix-ide.com/download-beta)!`
	const notifController = notifService.notify({
		severity: Severity.Info,
		message: message,
		sticky: true,
...
const performVoidCheck = async (
	...
	platonixUpdateService: IPlatonixUpdateService,
	...
	const res = await platonixUpdateService.check(explicit)
	if (!res) {
...
registerAction2(class extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		const platonixUpdateService = accessor.get(IPlatonixUpdateService)
		...
		const newController = await performVoidCheck(true, notifService, platonixUpdateService, metricsService, updateService)
...
class VoidUpdateWorkbenchContribution extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		@IPlatonixUpdateService platonixUpdateService: IPlatonixUpdateService,
		...
		const autoCheck = () => {
			performVoidCheck(false, notifService, platonixUpdateService, metricsService, updateService)
		}
