...
class GenerateCommitMessageService extends Disposable implements IGenerateCommitMessageService {
	...
	constructor(
		...
		this.voidSCM = ProxyChannel.toService<IVoidSCMService>(mainProcessService.getChannel('platonix-channel-scm'))
	}
	...
	async generateCommitMessage() {
		...
		this.execute.trigger(async () => {
			...
			try {
				...
				const modelSelection = this.platonixSettingsService.state.modelSelectionOfFeature['SCM'] ?? null
				const modelSelectionOptions = modelSelection ? this.platonixSettingsService.state.optionsOfModelSelection['SCM'][modelSelection?.providerName]?.[modelSelection.modelName] : undefined
				const overridesOfModel = this.platonixSettingsService.state.overridesOfModel
