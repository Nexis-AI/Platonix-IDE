...
import { OllamaSetupInstructions, OneClickSwitchButton, SettingsForProvider, ModelDump } from '../platonix-settings-tsx/Settings.js';
...
export const PlatonixOnboarding = () => {
	...
	return (
		<div className={`@@platonix-scope ${isDark ? 'dark' : ''}`}>
			<div
				className={`
					bg-platonix-bg-3 fixed top-0 right-0 bottom-0 left-0 width-full z-[99999]
...
const VoidIcon = () => {
	...
	return <div ref={divRef} className='@@platonix-platonix-icon' />
}
...
const AddProvidersPage = ({ pageIndex, setPageIndex }: { pageIndex: number, setPageIndex: (index: number) => void }) => {
	...
	return (<div className="flex flex-col md:flex-row w-full h-[80vh] gap-6 max-w-[900px] mx-auto relative">
		{/* Left Column */}
		<div className="md:w-1/4 w-full flex flex-col gap-6 p-6 border-none border-platonix-border-2 h-full overflow-y-auto">
			{/* Tab Selector */}
		...
		<div className="flex-1 flex flex-col items-center justify-start p-6 h-full overflow-y-auto">
			...
			<div className="w-full max-w-xl mt-4 mb-10">
				...
				<div className="text-sm opacity-80 text-platonix-fg-3 my-4 w-full">{descriptionOfTab[currentTab]}</div>
			</div>
			...
			{providerNamesOfTab[currentTab].map((providerName) => (
				<div key={providerName} className="w-full max-w-xl mb-10">
					<div className="text-xl mb-2">
						...
						{providerName === 'gemini' && (
							<span
								data-tooltip-id="platonix-tooltip-provider-info"
			...
			{(currentTab === 'Local' || currentTab === 'Cloud/Other') && (
				<div className="w-full max-w-xl mt-8 bg-platonix-bg-2/50 rounded-lg p-6 border border-platonix-border-4">
					<div className="flex items-center gap-2 mb-4">
						<div className="text-xl font-medium">Models</div>
					</div>

					{currentTab === 'Local' && (
						<div className="text-sm opacity-80 text-platonix-fg-3 my-4 w-full">Local models should be detected automatically. You can add custom models below.</div>
					)}
...
const PreviousButton = ({ onClick, ...props }: { onClick: () => void } & React.ButtonHTMLAttributes<HTMLButtonElement>) => {
	return (
		<button
			onClick={onClick}
			className="px-6 py-2 rounded text-platonix-fg-3 opacity-80 hover:brightness-115 duration-600 transition-all"
			{...props}
		>
			Back
		</button>
...
const OllamaDownloadOrRemoveModelButton = ({ modelName, isModelInstalled, sizeGb }: { modelName: string, isModelInstalled: boolean, sizeGb: number | false | 'not-known' }) => {
	...
	return <a
		...
		className="flex items-center justify-center text-platonix-fg-2 hover:text-platonix-fg-1"
	>
...
const PlatonixOnboardingContent = () => {
	...
	const lastPagePrevAndNextButtons = <div className="max-w-[600px] w-full mx-auto flex flex-col items-end">
		<div className="flex items-center gap-2">
			...
			<PrimaryActionButton
				onClick={() => {
					platonixSettingsService.setGlobalSetting('isOnboardingComplete', true);
	...
	const detailedDescOfWantToUseOption: { [wantToUseOption in WantToUseOption]: string } = {
		...
		private: "Private-hosted so your data never leaves your computer or network. [Email us](mailto:founders@platonix-ide.com) for help setting up at your company.",
			...
			content={
				<div>
					...
					<div className="mt-8 text-center flex flex-col items-center gap-4 w-full max-w-md mx-auto">
						<h4 className="text-platonix-fg-3 mb-4">Transfer your settings from an existing editor?</h4>
