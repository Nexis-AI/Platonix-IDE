...
/* eslint-disable */ // Void
import { VOID_CTRL_K_ACTION_ID, VOID_CTRL_L_ACTION_ID } from '../../../contrib/platonix/browser/actionIDs.js';
	...
	private render(): void {
		...
		const update = async () => {
			...
			if (this.contextService.getWorkbenchState() === WorkbenchState.EMPTY) {
				...
				openFolderButton.root.classList.add('platonix-openfolder-button')
				...
				openSSHButton.root.classList.add('platonix-openssh-button')
				...
				if (recentlyOpened.length !== 0) {
					...
					voidIconBox.append(
						...recentlyOpened.map((w, i) => {
							...
							const linkSpan = $('span');
							linkSpan.classList.add('platonix-link')
				...
				// button3.classList.add('platonix-settings-watermark-button')
